...
const data: { value: number; timestamp: string }[] = [
  {
    value: 500,
    timestamp: '10.30',
  },
...
const x = (_d: { value: number; timestamp: string }, i: number): number => i + 1;
const y = [(d: { value: number; timestamp: string }): number => d.value];

const generateTime = (): { timestamp: string[]; current: number[] } => {
  const now = new Date();
  return {
    timestamp: [
...
<template>
  <main class="relative flex h-screen">
    <section class="flex flex-col justify-start gap-10 bg-white text-slate-700 w-fit p-10 pr-14 pnsc-light overflow-auto">
    ...
    <UCard class="w-full overflow-y-auto" variant="soft">
      ...
      <section v-if="pageIndication == 'Dashboard'" class="flex flex-col gap-5">
        ...
        <div class="flex gap-1.5 flex-wrap justify-start">
          <UCard v-for="i in count" class="flex flex-col gap-5" :class="count < 3 ? 'w-full' : 'w-96'">
            <h1 class="text-xs pnsc-light mb-2">Oven {{ i }} {{ selectedItems }}</h1>
            <VisXYContainer :data="data" :height="180">
              <VisLine :x="x" :y="y" :lineWidth="2" />
              <VisAxis type="x" :x="x" :gridLine="false" label="Time" labelFontSize="11px" tickTextFontSize="12px" />
      ...
      <section v-if="pageIndication === 'Record'" class="flex items-start gap-2">
        ...
        <section class="flex-2 h-[calc(100vh-120px)] flex flex-col gap-5">
          <UCard class="h-full flex justify-center items-center">
            <UCard class="flex flex-col gap-5 w-[40rem]" variant="soft" v-if="showGraph">
              <VisXYContainer :data="data" :height="500" :yDomain="[0, 600]">
                <VisLine :x="x" :y="y" :lineWidth="2" />
                <VisAxis type="x" :x="x" label="Time" labelFontSize="11px" tickTextFontSize="12px" :numTicks="20" />
                <VisAxis type="y" :y="x" label="Temp(C)" labelFontSize="11px" tickTextFontSize="12px" :numTicks="10" />
              </VisXYContainer>
            </UCard>
